...
// (152:8) {#if component_name !== undefined && component_map.get(component_name) !== undefined}
...
// (150:7) {#each sample_row as { value, component }
...
// (140:5) {#each component_meta as sample_row, i}
...
// (112:6) {#if component_meta.length && component_map.get(components[0])}
...
// (100:3) {#each selected_samples as sample_row, i}
function create_each_block_1(ctx) {
	let if_block_anchor;
	let current;
	let if_block = /*sample_row*/ ctx[38][0] && create_if_block_3(ctx);
	...
	return {
		...
		p(ctx, dirty) {
			if (/*sample_row*/ ctx[38][0]) {
				if (if_block) {
					if_block.p(ctx, dirty);

					if (dirty[0] & /*selected_samples*/ 16384) {
						transition_in(if_block, 1);
					}
				} else {
...
// (180:3) {#each visible_pages as visible_page}
...
// (70:0) <Block  {visible}  padding={false}  {elem_id}  {elem_classes}  {scale}  {min_width}  allow_overflow={false}  container={false} >
...
//# sourceMappingURL=Index-0285e6c9.js.map
